...
def verify_file_structure():
    ...
    # 检查目录是否存在
    for directory in [upload_dir, pdf_upload_dir, pdf_output_dir]:
    ...
    return upload_dir, pdf_upload_dir, pdf_output_dir
...
def test_file_operations():
    ...
    try:
        ...
        # 验证目录结构
        upload_dir, pdf_upload_dir, pdf_output_dir = verify_file_structure()
        ...
        if os.path.exists(dest_path):
            logger.info(f"文件存在，大小: {os.path.getsize(dest_path)} 字节")
        else:
            logger.error("文件移动失败")
            return False
...
def main():
    """主测试函数"""
    logger.info("开始完整PDF翻译流程测试")
    
    try:
        success = test_file_operations()
        if success:
            logger.info("完整PDF翻译流程测试通过")
            print("SUCCESS: 完整PDF翻译流程测试通过")
            return True
        else:
            logger.error("完整PDF翻译流程测试失败")
            print("ERROR: 完整PDF翻译流程测试失败")
            return False
